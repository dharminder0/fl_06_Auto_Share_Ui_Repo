...
export class ComboChartService {
  ...
  createBarChart() {
    ...
    for (var i = 0; i < this.chartDataSource.data.length; i++) {
      ...
      var rect_bar = this.barChartGroup.rect(bar_x, bar_height+bar_y, bar_width, 0).attr({
        // fill: "#afd8f8",
      ...
      //   height: bar_height
      // }, 10000)
      rect_bar.animate({y: bar_y, height: bar_height}, 500)
    }
  }
  ...
  createLineChart() {
    ...
    var lineLength = Snap.path.getTotalLength(lineChartPath);
    ...
    lineChartDraw.animate({ strokeDashoffset: 0 }, 500);
  }
  ...
  createScatterChart() {
    ...
    for (var i = 0; i < this.chartDataSource.data.length; i++) {
      ...
      lineDraw.animate({ strokeDashoffset: 0, fill: "#ffffff" }, i*100);
    }
  }
